...
const size_t numThread = 8; // # of the thread
...
int main() {
    ...
    for (size_t i = 0; i < numThread; ++i) {
        pthread_join(pthID[i], (void*) &arrOut[i]);
...
void* plusTen(void* arg) {
    ...
    *ptrVal += 10;  // add 10
    ...
    // return ptrVal;
    /*
    int val = *((int*) arg);   // ptrVal is a pointer toward an integer
    printf("\n(thread) &val = %p", &val);       // print the address
    printf("\n(thread) input val = %d", val);   // print the value by dereference
    val += 10;  // add 10
    pthread_exit(val);
     */
}